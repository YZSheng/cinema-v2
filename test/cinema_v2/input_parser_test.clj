...
(deftest movie-parser-test
  ...
  (testing "single word movie with invalid row and columns"
    (is (= [nil "Row must be between 1 and 26 inclusive"] (sut/parse-movie "Inception a 10"))))
  ...
  (testing "single word movie with row and invalid columns"
    (is (= [nil "Column must be between 1 and 50 inclusive"] (sut/parse-movie "Inception 10 b"))))
  (testing "invalid both row and column input shows row error message"
    (is (= [nil "Row must be between 1 and 26 inclusive"] (sut/parse-movie "Inception a b"))))
